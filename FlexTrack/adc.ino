...
void SetupADC(void)
{
  #ifdef A0_MULTIPLIER //#ifdef, if the following was defined
    analogReference(DEFAULT); // it expecting no more than 5 volts 
    // Serial.println("Setup A0");
    pinMode(A0, INPUT); //setting it from A0 to an input 
  #endif
}

void CheckADC(void)
{
  if (millis() >= CheckADCChannels) //millis(): returns the number of milliseconds; number will overflow to zero 
  {
    #ifdef A0_MULTIPLIER
...
unsigned int ReadADC(int Pin, float Multiplier, unsigned int *Readings)
{
  ...
  Readings[4] = analogRead(Pin); //reads the value from the specified analog pin
